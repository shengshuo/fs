...
public class BuildDriver {
	...
	static String heartBeatName = "heartbeat.txt";
		...
		try {
			...
			while (keepListen) {
				if (checkHeartBeat()) {
					...
					PathMapBuilder.startBuidPath(new String[0]);
					keepListen = false;
				}
	...
	private static void readConfig() {		
		try {		
			...
			System.out.println(configProp.getProperty("rootDir"));
			System.out.println(configProp.getProperty("threadCount"));
		} catch (IOException ioe) {
			ioe.printStackTrace();
		} finally {
	...
	private static void writeConfig() {
		try {		
			configProp.setProperty("rootDir", "c:\\roottmp");
			configProp.setProperty("threadCount", "1");
			configOutput = new FileOutputStream(configFile);
			configProp.storeToXML(configOutput, "FileScanner properties");
		} catch (IOException ioe) {
			ioe.printStackTrace();
		} finally {
			if (configOutput != null) {
